...
_show_random_pokemon() {
    ...
    if [ "$shiny" = "$SHINY_RATE" ]; then
        cat "$POKEART_SHINY_DIR/$random_pokemon"
        echo "$random_pokemon (shiny)"
    else
        cat "$POKEART_REGULAR_DIR/$random_pokemon"
        echo "$random_pokemon"
    fi
}
...
_show_pokemon_by_name() {
    ...
    cat "$POKEART_REGULAR_DIR/$pokemon_name" 2> /dev/null || echo "Invalid pokemon"
}

_show_shiny_pokemon_by_name() {
    echo "$1 (shiny)"
    cat "$POKEART_SHINY_DIR/$1" 2> /dev/null || echo "Invalid pokemon"
}
